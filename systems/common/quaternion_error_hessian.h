...
/*!
 * Computes the gradient of the squared angle difference between two quaternions,
 * with respect to the elements of the first.
 * @param quat The first quaternion (size 4 vector). The gradient of the squared
 * angle difference is with respect to this quaternion.
 * @param quat_desired The second quaternion (size 4 vector).
 * @return The 4-element gradient vector of the squared angle difference.
*/
Eigen::VectorXd gradient_of_squared_quaternion_angle_difference(
    const Eigen::VectorXd& quat,
    const Eigen::VectorXd& quat_desired);
