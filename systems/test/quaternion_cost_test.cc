...
class QuaternionCostTest : public ::testing::Test {
 protected:
  ...
  std::vector<MatrixXd> UpdateQuaternionCosts(
      Eigen::VectorXd& x_curr, Eigen::VectorXd& x_des,
...
TEST_F(QuaternionCostTest, RealizedCostTest) {
  ...
  Eigen::Vector4d qd(q0.w(), q0.x(), q0.y(), q0.z());
  ...
  //q_vec << 0.972138, 0, 0.247621, 0; // from rollouts
  ...
  for (int i = -10; i <= 10; i++) {
    ...
    MatrixXd hessian =
        common::hessian_of_squared_quaternion_angle_difference(q_vec, qd);

    VectorXd xd(VectorXd::Zero(23));
    xd.segment(5, 4) = qd;

    VectorXd x_curr(VectorXd::Zero(23));
    x_curr.segment(5, 4) = q_vec;

    MatrixXd Q = UpdateQuaternionCosts(x_curr, xd, controller_options)[0].block(5, 5, 4, 4);



    Eigen::Quaterniond q_c3 = Eigen::AngleAxisd(i * 0.15, Eigen::Vector3d::UnitX()) *
                          Eigen::AngleAxisd(i * 0.15, Eigen::Vector3d::UnitY()) *
                          Eigen::AngleAxisd(0, Eigen::Vector3d::UnitZ());
    // Eigen::Quaterniond q_c3(0.987478, 0, 0.659032, 0);


    // Eigen::AngleAxisd dR(i * 0.2, Eigen::Vector3d::UnitY());
    // Eigen::Quaterniond dq(dR);

    //Eigen::Quaterniond q_perturbed = dq * q_c3;
    Eigen::Quaterniond q_perturbed = q_c3;
    Eigen::Vector4d q_perturbed_vec(q_perturbed.w(), q_perturbed.x(), q_perturbed.y(), q_perturbed.z());
    ...
    Eigen::AngleAxisd angle_axis(q0 * q_perturbed.inverse());
    double angle = angle_axis.angle();

    //std::cout << "cost diff: " << cost_hessian - cost_regularized << std::endl << std::endl;

    // std::cout << hessian << std::endl;
    //std::cout << Q << std::endl;
    // std::cout << "q_curr: " << q_perturbed_vec.transpose() << std::endl;
    // std::cout << "q_des: " << qd.transpose() << std::endl;
