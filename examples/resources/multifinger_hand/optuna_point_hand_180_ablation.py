...
def objective(trial):
    ...
    lambda_threshold = trial.suggest_int("lambda_threshold", 5, 60, step=5)
    eta_threshold = trial.suggest_int("eta_threshold", 1, 10)
    ...
    quat_weight = trial.suggest_int("quat_weight", 5, 100, step=5)
    ...
    for i in range(15):
        c3_options["c3_options"]["q_vector"][16+i] = 1
...
# ======================================================================
    ...
    try:
        for line in iter(process.stdout.readline, ''):
            ...
            if final_match:
                final_score_thresh = float(final_match.group(1))
...
# =====================================================================================================
    ...
    # Run again with no thresholds
    with open(CONTORLLER_PARAMS, "r") as f:
        c3_options = yaml.safe_load(f)
        
    c3_options["c3_options"]["lambda_threshold"] = [0] * (4 * n_contacts)
    c3_options["c3_options"]["eta_threshold"] = [0] * (4 * n_contacts)
   ...
   # Construct and execute the bazel command
    ...
    try:
        for line in iter(process.stdout.readline, ''):
            ...
            final_match = re.search(r"FINAL_METRIC:\s*([0-9.]+)", line)
            if final_match:
                final_score_no_thresh = float(final_match.group(1))
    ...
    if final_score_no_thresh is None:
        raise optuna.TrialPruned("Could not find FINAL_METRIC in output.")
    
    trial.set_user_attr("final_score_thresh", final_score_thresh)
...
def log_best_callback(study, trial):
    ...
    # Retrieve the individual components (default to None if missing)
    score_thresh = trial.user_attrs.get("final_score_thresh")
    score_no_thresh = trial.user_attrs.get("final_score_no_thresh")
    score_diff = trial.user_attrs.get("score_diff")

    # 1. LOG EVERY TRIAL WITH METRIC < 70
    if trial.value < 70:
        ...
        with open("examples/resources/multifinger_hand/optuna_point_hand_180/sub_70_trials_ablation_wG_final3.txt", "a") as f:
            ...
            f.write(f"  score_diff: {score_diff}\n")
            f.write("Parameters:\n")
