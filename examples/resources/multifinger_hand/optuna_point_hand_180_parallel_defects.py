...
# python3 examples/resources/multifinger_hand/optuna_point_hand_180_parallel_defects.py
...
def objective(trial):
    ...
    finger_position_weight = trial.suggest_int("finger_position_weight", 1000, 10000, step=1000)
    cube_position_weight = trial.suggest_int("cube_position_weight", 1000, 20000, step=1000)
    ...
    # =========================================================================
    # 2. MS-iC3 Options & Defect Minimization Hyperparameters
    # =========================================================================
    # num_iters = trial.suggest_categorical("num_iters", [8, 10, 12])
    num_iters = 10
    num_segments = trial.suggest_categorical("num_segments", [60, 70])
    
    alpha_ee = trial.suggest_int("alpha_ee", 0, 100)
    # alpha_ee_step = trial.suggest_float("alpha_ee_step", 0.02, 0.2, step=0.02)
    alpha_ee_step = 0
    alpha_object = trial.suggest_int("alpha_object", 0, 100)
    # alpha_object_step = trial.suggest_float("alpha_object_step", 0.02, 0.2, step=0.02)
    alpha_object_step = 0

    value_function_scaling = trial.suggest_int("value_function_scaling", 0, 100)

    # vf_trust_region_weight = trial.suggest_float("vf_trust_region_weight", 0.1, 50.0, step=1.0)
    vf_trust_region_weight = 10
    ...
    terminal_slack_cube = trial.suggest_int("terminal_slack_cube", 1000, 50000, step=1000)
    terminal_slack_quat = trial.suggest_int("terminal_slack_quat", 500, 50000, step=500)
    ...
    ic3_options["alpha_ee"] = alpha_ee / 100
    ic3_options["alpha_ee_step"] = alpha_ee_step / 100
    ic3_options["alpha_object"] = alpha_object / 100
    ic3_options["alpha_object_step"] = alpha_object_step / 100

    ic3_options["value_function_scaling"] = value_function_scaling
    ...
    # =========================================================================
    # Find the final completed iteration
    ...
    cost_total_quat = 1.0 * total_quat_defect
    cost_max_quat = 2.0 * max_quat_defect
    cost_total_obj = 500.0 * total_obj_defect
    cost_max_obj = 500.0 * max_obj_defect
    cost_total_ee = 1000.0 * total_ee_defect
    cost_target_angle = 0.5 * target_angle_err
    cost_pos = 100.0 * pos_err
...
def log_best_callback(study, trial):
    ...
    os.makedirs("examples/resources/multifinger_hand/optuna_point_hand_180_parallel", exist_ok=True)
    attrs = trial.user_attrs
    
    # Save sub-threshold trials
    if trial.value < 50:
        with open("examples/resources/multifinger_hand/optuna_point_hand_180_parallel/sub_50_defect_trials.txt", "a") as f:
    ...
    if study.best_trial.number == trial.number:
        ...
        with open("examples/resources/multifinger_hand/optuna_point_hand_180_parallel/best_params_180_parallel_defects.txt", "w") as f:
            f.write("=========================================\n")
...
if __name__ == "__main__":
    ...
    if "microsoft" in platform.release().lower():
        STORAGE_URL = "sqlite:////home/ttesc255/optuna_data/optuna_results_180_parallel_defects.db"
    else:
        STORAGE_URL = "sqlite:///examples/resources/multifinger_hand/optuna_point_hand_180_parallel/optuna_results_180_parallel_defects.db"
    ...
    study = optuna.create_study(
        study_name="MSiC3_point_hand_180_parallel_defects",
        storage=storage,
        load_if_exists=True,
        sampler=sampler,
        direction="minimize"
    ...
    print(f"Starting Optuna study: MSiC3_point_hand_180_parallel_defects (Worker {worker_id})")
